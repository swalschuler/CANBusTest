...
void CAN_1_ReceiveMsg(uint8 rxMailbox) 
{
    ...
    #endif /* CY_PSOC3 || CY_PSOC5 */
        {
            ...
            //CAN_1_GET_RX_ID(rxMailbox);
            int test = CAN_1_GET_RX_ID(rxMailbox);
            //CAN_1_RX_STRUCT packet = CAN_1_RX[rxMailbox];
            //CAN_1_RX_ID_REG(i) 
            can_receive(CAN_1_RX[rxMailbox]);
            if (test == 0x300) // might need to change for endinaness
            {
                GREEN_Write(1);
                CyDelay(500);
                GREEN_Write(0);
